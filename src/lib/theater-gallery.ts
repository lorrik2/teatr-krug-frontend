/** Все фото театра для фотогалереи */
export const theaterGalleryImages = [
  { src: "/fon/1.jpg", alt: "Интерьер театра" },
  { src: "/fon/2.jpg", alt: "Зрительный зал" },
  { src: "/fon/3.jpg", alt: "Сцена" },
  { src: "/fon/4.jpg", alt: "Фойе" },
  { src: "/fon/6.jpg", alt: "Зал" },
  { src: "/fon/7.jpg", alt: "Закулисье" },
  { src: "/fon/8.jpg", alt: "Фасад театра" },
  { src: "/fon/12.jpg", alt: "Зрительный зал" },
  { src: "/fon/13.jpg", alt: "Фойе" },
  { src: "/fon/14.jpg", alt: "Закулисье" },
  { src: "/fon/22.jpg", alt: "Гримёрки" },
  { src: "/fon/23.jpg", alt: "Закулисье" },
  { src: "/fon/56.jpg", alt: "Интерьер" },
  { src: "/fon/67.jpg", alt: "Сцена" },
  { src: "/fon/145.jpg", alt: "Театр" },
];

export const GALLERY_PAGE_SIZE = 12;
